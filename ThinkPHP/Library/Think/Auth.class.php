...
class Auth{
    ...
    public function getGroups($uid) {
        ...
        $user_groups = M()
            ...
            ->field('id,rules,uid,group_id,title')->select();
    ...
    protected function getAuthList($uid,$type) {
        ...
        //读取用户组所有权限规则
        $rules = M()->table($this->_config['AUTH_RULE'])->where($map)->field('param,name,auth_name')->select();
        ...
        foreach ($rules as $rule) {
            if (!empty($rule['condition'])) { //根据condition进行验证
                ...
                $command = preg_replace('/\{(\w*?)\}/', '$user[\'\\1\']', $rule['param']);
                //dump($command);//debug
                @(eval('$condition=(' . $command . ');'));
                if ($condition) {
                    $authList[] = strtolower($rule['auth_name']);
                }
            } else {
                //只要存在就记录
                $authList[] = strtolower($rule['auth_name']);
